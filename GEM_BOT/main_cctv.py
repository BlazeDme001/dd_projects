...
def main():
    ...
    try:
        ...
        pass
    # result_df = pd.read_csv('csv_files\\bid_data_cctv.csv')
    filtered_df = result_df[result_df['new_itm'].str.contains(r'\bCCTV\b', case=False, regex=True)]
    filtered_df = filtered_df[~filtered_df['new_itm'].str.contains(r'\bDSLR\b', case=False, regex=True)]
    filtered_df.to_csv(os.path.join(os.getcwd(),'csv_files','bid_data_cctv.csv'))
