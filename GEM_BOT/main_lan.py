...
def main():
    ...
    # result_df = pd.read_csv('csv_files\\bid_data_lan.csv')
    # result_df['new_itm'] = result_df['new_itm'].fillna(result_df['Items'])
    filtered_df = result_df[result_df['new_itm'].str.contains('LAN', case=False, regex=False)]
    filtered_df = result_df[result_df['new_itm'].str.contains(r'\bLAN\b', case=False, regex=True)]
    # filtered_df = result_df[result_df['new_itm'].str.contains('LAN', case=False, regex=False)]
    filtered_df.to_csv(os.path.join(os.getcwd(),'csv_files','bid_data_lan.csv'))
    # filtered_df['new_itm'].to_csv('lan.csv')


# if __name__ == '__main__':
#     while True:
# main()
#         print('BOT will start after 5 hours')
#         time.sleep(18000)
