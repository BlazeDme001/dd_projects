...
def main():
    ...
    bid_search.click()
    bid_search.send_keys('Unmanned Aerial Vehicle')
    # bid_search.send_keys('GEM/2023/B/3549102')
    ...
    # except:
    #     try:
    #         total_pages = driver.find_element(By.XPATH, '//*[@id="light-pagination"]/a[1]')
    #         tp = total_pages.text
    #     except:
    #         tp = 1
    count = 7
    while count:
        count -= 1
        try:
            total_pages = driver.find_element(By.XPATH, f'//*[@id="light-pagination"]/a[{count}]')
            if total_pages.text != 'Next':
                break
        except:
            pass
    tp = total_pages.text
    ...
    try:
        ...
        pass
    # result_df = pd.read_csv('csv_files\\bid_data_Unmanned Aerial Vehicle.csv')
    filtered_df = result_df[result_df['new_itm'].str.contains(r'\bUnmanned Aerial Vehicle\b', case=False, regex=True)]
    filtered_df.to_csv(os.path.join(os.getcwd(),'csv_files','bid_data_uav2.csv'))
