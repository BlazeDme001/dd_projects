from flask import Flask, request, render_template, redirect, url_for, send_file, session, make_response, jsonify, flash, Response
import os
...
from face_capture import face_capture_bp
...
def login():
    try:
        ...
        print('Error during logout, continuing...')
        pass
    if request.method == 'POST':
        ...
        if user:
            ...
            query = f"""SELECT 1 FROM tender.user_faces WHERE user_id = '{username}'"""
            face_record = db.get_data_in_list_of_tuple(query)  # Make sure this returns a result or empty
            
            session['logged_in'] = True
            session['username'] = user.username
            session['profile'] = user.profile
            
            if not face_record:  # If no face is registered
                return redirect(url_for('face_capture_bp.face_capture_page'))
...
def tenders():
    ...
    current_user = session.get('username')  # Get the current user object
